...
http {
  ...
  server {
    ...
    {{- $writeUrl    := printf "http://%s.%s.svc.%s:3100" $writeHost   .Values.namespace .Values.global.clusterDomain }}
    {{- $readUrl     := printf "http://%s.%s.svc.%s:3100" $readHost    .Values.namespace .Values.global.clusterDomain }}
    {{- $backendUrl  := printf "http://%s.%s.svc.%s:3100" $backendHost .Values.namespace .Values.global.clusterDomain }}
